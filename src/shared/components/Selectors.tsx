        ...
        {tiers.map(t => (
          <button 
            ...
            className={`w-8 h-8 rounded-lg flex items-center justify-center text-[8px] font-black uppercase tracking-tighter transition-all ${
              current === t ? 'bg-primary-blue text-white shadow-lg' : 'text-text-light hover:bg-white/5'
            }`}
          >
            {t[0]}
          </button>
      ...
      {tiers.map(t => (
        <button 
          ...
          className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
            current === t ? 'bg-primary-blue text-white shadow-xl shadow-blue-500/10' : 'text-text-light hover:text-text-main'
          }`}
        >
          {t}
        </button>
      ...
      {options.map(opt => (
        <button 
          ...
          className={`p-2 rounded-lg transition-all ${
            current === opt.id ? 'bg-primary-blue text-white shadow-md' : 'text-text-light hover:text-text-main hover:bg-white/10'
          }`}
        >
          <opt.icon size={14} />
        </button>
